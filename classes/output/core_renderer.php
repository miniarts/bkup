...
class core_renderer extends \core_renderer {
    ...
    /*
        checkbox can't be checked by default in the setting, so I had to use the value the other way around
    */
    public function navbar_inverse(){
        $classnames = '';

        if(!empty($this->page->theme->settings->invert)){
            $classnames .= 'navbar-light bg-white';
        }else{
            $classnames .= 'navbar-inverse navbar-dark bg-primary';
        }
        
        return $classnames;
    }

    public function full_header_no_title() {
        $html = html_writer::start_tag('header', array('id' => 'page-header', 'class' => 'row'));
